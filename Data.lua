...
function Data:RewardIsUpgrade(questID)
	...
	if equipSlot and invtype_locations[equipSlot] then
		...
		return isUpgrade
	-- -- Disabled for now, not easy to find if upgrade for non-equipped artifact or if 3rd slot is unlocked
	-- elseif relicTypes and relicTypes[itemID] then 
	-- 	local isUpgrade = false
	-- 	local relicType = relicTypes[itemID]
	-- 	local currentItem = GetInventoryItemLink("player", INVSLOT_MAINHAND)
	--
	-- 	for i=1,3 do
	-- 		local _, relicLink = GetItemGem(currentItem, i)
	-- 		if relicLink then
	-- 			local relicID = tonumber( relicLink:match("item:(%d+):") )
	-- 			if relicType == relicTypes[ relicID ] then
	-- 				local relicIlvl = select(4, GetItemInfo(relicLink))
	-- 				if ilvl > relicIlvl then
	-- 					isUpgrade = true
	-- 				end
	-- 			end
	-- 		end
